...
__global__ void Blelloch_Exclusive_Scan(ll *d_in, ll* d_out)
{
    ...
    // At the end of reduction, the last element of each block conatins the Maximum of all elements in that block
    // We store these block-wise maximum_number in d_out
    if(tid == (Block_Size - 1))
    {
...
// This GPU kernel compare the value d_out[id] to all values in the (id)th block of d_in
